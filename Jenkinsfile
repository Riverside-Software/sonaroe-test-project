...
node ('EC2-EU1B') {
  ...
  if ("master" == env.BRANCH_NAME) {
    bat "Z:\\Tools\\sonar-scanner-2.8\\bin\\sonar-scanner.bat -Dproject.settings=sonar-project1.properties"
  } else {
    withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'GitHub-GQuerret', usernameVariable: 'GH_LOGIN', passwordVariable: 'GH_PASSWORD']]) {
      bat "Z:\\Tools\\sonar-scanner-2.8\\bin\\sonar-scanner.bat -Dproject.settings=sonar-project2.properties -Dsonar.analysis.mode=issues -Dsonar.github.pullRequest=${env.BRANCH_NAME.substring(3)} -Dsonar.github.repository=Riverside-Software/sonaroe-test-project -Dsonar.github.oauth=${env.GH_PASSWORD}"
    }
  }
}
