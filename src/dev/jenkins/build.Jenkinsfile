            ...
            withSonarQubeEnv('RSSW') {
              withEnv(["SONAR_USER_HOME=/tmp"]) {
                if ('main' == env.BRANCH_NAME) {
                  sh "sonar-scanner -Dproject.settings=src/dev/sonar/sonar-project.properties -Dsonar.branch.name=$BRANCH_NAME"
                } else {
                  sh "sonar-scanner -Dproject.settings=src/dev/sonar/sonar-project.properties -Dsonar.pullrequest.branch=$BRANCH_NAME -Dsonar.pullrequest.base=main -Dsonar.pullrequest.key=$BRANCH_NAME"
                }
              }
            }
          }
