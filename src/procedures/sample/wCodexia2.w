...
&Scoped-Define ENABLED-OBJECTS RECT-27 COMBO-BOX-Application 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Application 
...
DEFINE VARIABLE COMBO-BOX-Application AS CHARACTER FORMAT "X(256)":U 
     ...
     DROP-DOWN-LIST
     SIZE 77.4 BY 1 TOOLTIP "S‚lectionner l'application" NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN resizeObject IN h_bCatalogue ( 5.67 , 36.00 ) NO-ERROR.
       ...
       RUN repositionObject IN h_bCatalogue2 ( 9.57 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bCatalogue2 ( 10.95 , 36.00 ) NO-ERROR.
...
DO i = 1 TO NUM-ENTRIES(pcListeChampBrowse) :
    /* modification du label si libell‚ renseign‚ */
    ...
    CASE ENTRY(2,cDataType,CHR(4)) :
        WHEN "character" THEN 
            /* Porte le format … x(80) lorsque par d‚faut il est … plus de 100 */
...
DEFINE VARIABLE hBrowse AS HANDLE     NO-UNDO.
 ...
 /* resize par d‚faut sur la largeur de la  window */ 
...
DEFINE VARIABLE hBrowse AS HANDLE     NO-UNDO.
  ...
  IF VALID-HANDLE(h_bCatalogue2) THEN RUN destroyObject IN h_bCatalogue2. 
  ...
  IF (ttAppliGed.AffDossier EQ 1) THEN DO:
     ...
     END.
     ELSE DO:
         RUN repositionObject IN h_bcatalogue2 ( 1.00  , 9.57 ) NO-ERROR.
         RUN resizeObject IN h_bcatalogue2     ( 10.86 , 30.60 ) NO-ERROR.
     END.
  END.
...
/*-------- Affiche le browse ------------- */
/* récupère les champs,labels et formats du browse codexia et mise et cr‚ation du browse */
...
/*-------- Affiche le browse ------------- */
/* r‚cupŠre les champs,labels et formats du browse codexia et mise et cr‚ation du browse */
RUN ref/cdxChampBrowseCodexia.p (INPUT  cCodeAppliGed     ,
                           INPUT  "b2-" + cNomEcran         ,
                           OUTPUT cListeChampBrowse ,
                           OUTPUT cListeLabelBrowse ,
...
CASE ttAppliGed.AffDossier :
  WHEN 1 THEN  /* si pr‚sentation tableau */
  DO:
  ...
  END.
  WHEN 2 THEN  /* pr‚sentation uniquement du browse */
  DO:
...
------------------------------------------------------------------------------*/
    ...
    /* Recherche de la valeur de la cl‚ dans le cfginireg */
    ASSIGN cSection = "wCodexia_" + cCodeAppliGed .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cdxMajDossier2 wWin 
PROCEDURE cdxMajDossier2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE i               AS INTEGER    NO-UNDO.
     /* Permet la gestion d'un écran <> entre creation et modification */
    DEFINE VARIABLE cPrefixe        AS CHARACTER  NO-UNDO INIT "g3-".
    ...
    IF pcMaj THEN DO :
        ...
        DO i = 1 TO 2 :        
            ...
            FOR EACH ttChampCodexia NO-LOCK BREAK BY ttChampCodexia.NomChamp :
                IF FIRST ( ttChampCodexia.NomChamp) THEN 
                   ASSIGN cColonnes = ttChampCodexia.NomChamp 
                          cValeurs  = DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue3, 
                                                       INPUT ttChampCodexia.NomChamp ) .
                ELSE 
                   ASSIGN cColonnes = cColonnes + CHR(1) + ttChampCodexia.NomChamp 
                          cValeurs  = cValeurs  + CHR(1) + 
                                      DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue3, 
                                                       INPUT ttChampCodexia.NomChamp ).
            END.
            IF  cColonnes = ""  THEN   cPrefixe = "g3-" .
                        ELSE i = 2.     
            */
        END.        
    END.
    ELSE ASSIGN cColonnes = '' cValeurs = '' .
    /*
    RUN smt/gUpdateSsDos.w ( INPUT INT(DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue3,INPUT "nodossier" )) , 
                             INPUT        cPrefixe + cNomEcran ,
                             INPUT        pcMaj        ,
                             OUTPUT       cValeurCle3  ,
                             INPUT-OUTPUT cColonnes    ,
                             ...
                             OUTPUT       lStatut      ).
    */
    IF cValeurCle1 <> "" /*AND cValeurCle3 <> "" */ THEN DO :
        
        IF pcMaj THEN DO :
            /*
            ASSIGN hHandle = DYNAMIC-FUNCTION('getASHandle':U IN h_dCatalogue3).  

            RUN CdxTransactionUpdate IN hHandle 
                ( INPUT DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue3, INPUT "NoDossier" )  ,
                  INPUT cColonnes                                                               ,
                  INPUT cValeurs                                                                ) .

            { lib/ASDisconnect.i h_dCatalogue3 hHandle }
            */
        END.
        ELSE DO :
            ASSIGN cValeurCle2   = DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue2, INPUT cChampCle2 )
                   cCle          = "KY"  + CHR(1) + cChampCle1 + CHR(1) + cValeurCle1 + CHR(1) 
                                 + "NI2" + CHR(1) + cChampCle2 + CHR(1) + cValeurCle2 + CHR(1)
        ...
        RUN CdxUpdateParent IN hHandle 
            ( INPUT (IF pcMaj THEN DYNAMIC-FUNCTION('columnValue':U IN h_dCatalogue3, INPUT "NoDossier") ELSE cNoDossier)).

        { lib/ASDisconnect.i h_dCatalogue3 hHandle }


        /* DYNAMIC-FUNCTION('openQuery':U IN h_dCatalogue2) . */
        DYNAMIC-FUNCTION('openQuery':U IN h_dCatalogue3) .*/
    END.
...
DO:
   ...
   /*------------- Contr“le position objet-----------------------*/
    /* Si viewer positionn‚ sur tool bar ou combo application */
    /*IF iViewerRow < 3.86 THEN 
        ASSIGN lPositionObjet = FALSE .
    
    ELSE 
    /* si browse positionn‚ sur viewer */
    ...
    ELSE 
     ...
     /* si viewer positionn‚ sur le browse */
     ELSE 
...
/* true */
    ...
    ELSE
        DYNAMIC-FUNCTION("phxMessage",INPUT "Codexia"
                                     ,INPUT 529  /* La position du tableau et le d‚tail du tableau est incorrecte */
                                     ,INPUT ""
                                     ,INPUT "INFORMATION"
                                     ,INPUT "OK"). 
...
------------------------------------------------------------------------------*/
  DISPLAY COMBO-BOX-Application 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE RECT-27 COMBO-BOX-Application 
      WITH FRAME fMain IN WINDOW wWin.
...
DEFINE VARIABLE hSdoServer          AS HANDLE        NO-UNDO.
  ...
  DO WITH FRAME {&FRAME-NAME} :
      ...
      /* Recup‚rer la liste des code AppliGed */
      ...
      /* ... tri d‚fini car non param‚trable ...*/
      ASSIGN cTri = "BY Catalogue.NoOrdreAff DESCENDING "  .              

     

/*** Pas ici dans initaff 
      DYNAMIC-FUNCTION('setQuerySort':U  IN h_dCatalogue2, INPUT cTri ).  
      DYNAMIC-FUNCTION('openQuery':U IN  h_dCatalogue2).
***/
      ...
      /* si pas autoriser … modifier de dessign griser tout le menu popup */  
      ASSIGN ldroitDesign = DYNAMIC-FUNCTION("getDroitFonction", "DESIGN"). 
      IF NOT ldroitDesign OR ttAppliGed.AffDossier = 2 /* pr‚sentation uniquement du browse */ THEN
      DO:
